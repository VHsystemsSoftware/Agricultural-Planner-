...
public interface ILayerService
{
	...
	Task UpdateLayerIsBufferAsync(EnabledDTO enabledDto);
...
public class LayerService : ILayerService
{
	...
	public async Task UpdateLayerIsBufferAsync(EnabledDTO enabledDto)
	{
		var layer = await _unitOfWork.Layer.GetByIdAsync(enabledDto.Id);
		if (layer == null)
			throw new KeyNotFoundException("Rack not found");

		var layers = _unitOfWork.Layer.Query(x => x.RackId == layer.RackId);
		foreach (var item in layers)
		{
			item.IsBufferLayer = false;
			_unitOfWork.Layer.Update(layer);
		}
	
		layer.IsBufferLayer = enabledDto.Enabled;
		_unitOfWork.Layer.Update(layer);
