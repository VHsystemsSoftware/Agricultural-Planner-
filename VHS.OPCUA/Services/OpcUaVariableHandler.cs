...
public class OpcUaVariableHandler
{
	...
	private bool? _retrySeeding = null;

	private readonly List<OPCAlarm> _alarms = new();
		...
		MonitoredItemNotification min)
	{
		try
		{
			...
			switch (compName)
			{
				...
				case "General.AlarmActiveFloor":
				case "General.AlarmActiveGerminationArea":
				case "General.AlarmActiveHarvestingStation":
				case "General.AlarmActivePaternoster":
				case "General.AlarmActiveSeedingStation":
				case "General.AlarmActiveTransplantStation":
				case "General.AlarmActiveTransportFromPater":
				case "General.AlarmActiveTransportToPater":
				case "General.AlarmActiveWashingStation":
				case "General.AlarmActiveGrowLineInput":
				case "General.AlarmActiveGrowLineOutput":
				case "General.AlarmActiveRackArea1":
				case "General.AlarmActiveRackArea2":
				case "General.EmergencyStop":
				case "General.ControlledStop":
				case "General.ForcedStop":
					await HandleGeneralAlarmAsync(server, min, compName);
					break;
	...
	private async Task HandleGrowLineAcceptedAsync(OpcUaServer server, MonitoredItemNotification min)
	{
		...
		{
			_logger.LogInformation($"HandleGrowLineInputTrayInfoAccepted - Request In feed, accepted: {min.Value}");
			...
			if (isTrayInfoAccepted == true)
			{
				_logger.LogInformation($"HandleGrowLineInputTrayInfoAccepted - Accepted trayId: {trayId}");

			}
			else
			{
				_logger.LogInformation($"HandleGrowLineInputTrayInfoAccepted - Not accepted trayId: {trayId}");
			}
		}
		else
		{
	...
	{
		if (min.Value != null && min.Value.Value is float floatValue)
		{
			_logger.LogInformation($"HandleHarvesterAccepted - Request In feed, weight kg: {floatValue}");

			(DataValue readItemTrayId, uint trayId, string readNode) = await GetTrayIdWeight(server);

			if (trayId != 0)
			{
				_logger.LogInformation($"HandleHarvesterAccepted - Request weightTrayId: {trayId}");
				bool? isPlannerInControl = GetIsPlannerInControl(server, Component.Harvester).Result;
				...
				_logger.LogInformation($"HandleHarvesterAccepted - Updated batch grow planner weightTrayId: {trayId}");
			}
			else
			{
				_logger.LogInformation($"HandleHarvesterAccepted - Unknown weightTrayId: {trayId}");
			}
		}
	...
	private async Task HandlePaternosterRequestedAsync(OpcUaServer server, MonitoredItemNotification min)
	{
		...
		{
			_logger.LogInformation($"HandlePaternosterRequestedAsync - ToDo: False value must be handled");
		}
	}
	...
	private async Task HandleSeedingRequestedAsync(OpcUaServer server, MonitoredItemNotification min)
	{
		...
		if (isRequest == true)
		{
			...
			if (trayId > 0)
			{
				...
				// Check if there is an retry loop already started, if null there is not
				if (_retrySeeding == null)
				{
					// Set the check to true to staat the retry loop;
					_retrySeeding = true;

					// While the retry loop is true, we will handle the request and check if there is a job available
					while (_retrySeeding == true)
					{
						await HandleSeedJobReqeust(server, min, trayId, requestMessage);

						// if the check is not positive, still no job, than wait x seconds before retrying
						if (_retrySeeding == true)
						{
							Thread.Sleep(5000);
						}
					}
	...
	private async Task HandleSeedJobReqeust(OpcUaServer server, MonitoredItemNotification min, uint trayId, SeedingTrayRequest requestMessage)
	{
		string response = await GetResponseFromAPI(requestMessage, trayId);
	...
	{
		//Skip tray info accepted, just continue
		bool isTrayInfoAccepted = true;// min.Value.GetValueOrDefault<bool>();
	...
	public void AddOrUpdate(List<OPCAlarm> list, OPCAlarm newItem)
	{
		var existingItem = list.FirstOrDefault(i => i.Name == newItem.Name);
	...
	private async Task HandleGeneralAlarmAsync(OpcUaServer server, MonitoredItemNotification min, string compName)
	{
		try
		{
			string variableName = compName.Split('.').Last();

			if (Enum.TryParse<MessageType>(variableName, out var eventType))
			{
				_logger.LogInformation("Handling General Alarm: {AlarmName}", variableName);
				bool isActive = min.Value.GetValueOrDefault<bool>();

				AddOrUpdate(_alarms, new OPCAlarm() { Name = variableName.Replace("AlarmActive", ""), Status = isActive });
				var alarmsJson = JsonConvert.SerializeObject(_alarms, Formatting.Indented);
				var alarmData = new GeneralAlarmRequestData
				{
					Value = new VarAlarmIsActive
					{
						Node = server.GetVariableNodeName(min.ClientHandle.ToString()),
						Value = alarmsJson,
						OPCDateTime = min.Value.SourceTimestamp.ToUniversalTime()
				...
				string jsonRequest = JsonConvert.SerializeObject(requestPayload, Formatting.Indented);

				string url = $"{_apiBase}/{(int)eventType}/0";
				var response = await PostJsonAsync(url, jsonRequest);

				_logger.LogInformation("Alarm response: {response}", response);
			}
			else
			{
				_logger.LogWarning("Could not parse variable name '{VarName}' to a known MessageType.", variableName);
			}
		}
		catch (Exception ex)
		{
			_logger.LogError(ex, "Error handling general alarm for {CompName}", compName);
			SentrySdk.CaptureException(ex);
		}
	}
	...
	private static async Task<(DataValue readItemTrayId, uint trayId, string readNode)> GetTrayIdWeight(OpcUaServer server)
	{
		string readNode = $"ns={server.NamespaceIndex};s=::ExtComm:STS.Harvester.WeightTrayId";
		ReadResponse readResultInfeed = await server.ReadNodeDataAsync([readNode]);
		DataValue readItemTrayId = readResultInfeed.Results[0];
		uint trayId = readItemTrayId.GetValueOrDefault<UInt32>();
		return (readItemTrayId, trayId, readNode);
